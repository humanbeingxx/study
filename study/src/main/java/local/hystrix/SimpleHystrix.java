...
public class SimpleHystrix {
    ...
    public static HystrixCommand<String> createSimpleCommand(String commandName) {
        return new HystrixCommand<String>(HystrixCommandGroupKey.Factory.asKey(commandName)) {

            @Override
            protected String run() throws Exception {
                return "Hello " + getCommandGroup().name() + " @ " + System.currentTimeMillis();
            }
        };
    }

    public static HystrixObservableCommand<String> createSimpleObservableCommand(String commandName) {
        return new HystrixObservableCommand<String>(HystrixCommandGroupKey.Factory.asKey(commandName)) {
            @Override
    ...
    private static SyncOnSubscribe<Integer, String> createSyncOnSubscribe() {
        return SyncOnSubscribe.createStateful(
                new Func0<Integer>() {
                    @Override
                    public Integer call() {
                        return 10;
                ...
                new Func2<Integer, Observer<? super String>, Integer>() {
                    @Override
                    public Integer call(Integer integer, Observer<? super String> observer) {
                        if (integer <= 0) {
                            observer.onCompleted();
                            return -1;
                        }
                        observer.onNext("subscribe on " + integer + " @ " + System.currentTimeMillis());
                        return integer - 1;
                    }
                }
        );
    ...
    public static HystrixObservableCommand<String> createSingleObservableCommand(String commandName) {
        return new HystrixObservableCommand<String>(HystrixCommandGroupKey.Factory.asKey(commandName)) {
            @Override
            protected Observable<String> construct() {
    ...
    public static HystrixObservableCommand<String> createSimpleObservableFallBack(String commandName) {
        return new HystrixObservableCommand<String>(HystrixCommandGroupKey.Factory.asKey(commandName)) {

            @Override
            protected Observable<String> construct() {

                return Observable.create(createSyncOnSubscribeRandomException());
            }
